...
export function getPenPlotGcode(props) {
...
export function getPenPlotGcodeFromOp(settings, opIndex, op, geometry, openGeometry, tabGeometry, showAlert, done, progress) {
    let ok = true;

    if (op.type !== 'Pen Plot' && op.type !== 'Laser Fill Path') {
...
/*         useBlower: op.useBlower ? {
        ...
        gcodeToolOn: settings.gcodePenOn,
        gcodeToolOff: settings.gcodePenOff,
